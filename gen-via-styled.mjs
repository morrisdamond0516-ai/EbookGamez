...
// --- STEP 1: Create temp draft ebooks & generate via test-style-d -----------
const CAMPAIGNS = [
  {
    key: 'reading-pass',
    title: 'The Reading Pass',
    genre: 'fiction',
    topic: 'A private cathedral library, fireplace glow, leather chairs, antique gold candlelight, deep mahogany and shadow',
  ...
  },
  {
    ...
    genre: 'history',
    topic: 'Ancient war room with fantasy map, gold candelabra, brass compass and scrolls, dark stone fortress, deep shadow',
  ...
  },
  {
    ...
    genre: 'mystery',
    topic: 'A single glowing ancient book on dark mahogany pedestal, golden light erupting from pages into pure darkness',
...
// Helper: create a draft in the DB
async function createDraft(c) {
  const r = await fetch(`${APP}/api/content-studio/drafts`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ title: c.title, genre: c.genre, topic: c.topic, description: c.topic }),
  });
  if (!r.ok) throw new Error(`Create draft failed: ${r.status} ${await r.text()}`);
  const data = await r.json();
  console.log(`  Created draft id=${data.id}`);
  return data.id;
}

// Helper: trigger test-style-d generation
async function generateCover(draftId) {
  const r = await fetch(`${APP}/api/content-studio/regenerate-cover-with-style/${draftId}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ styleId: 'test-style-d' }),
  });
  if (!r.ok) throw new Error(`Cover gen failed: ${r.status} ${await r.text()}`);
  const data = await r.json();
  console.log(`  Cover URL: ${data.coverUrl}`);
  return data.coverUrl;   // e.g. "/uploads/covers/ai-bg-test-style-d-1234567.png"
}

// Helper: delete temp draft
async function deleteDraft(draftId) {
  // Use direct DB delete via a temp fetch — try admin delete endpoint
  const r = await fetch(`${APP}/api/content-studio/drafts/${draftId}`, { method: 'DELETE' });
  if (r.ok) console.log(`  Deleted draft ${draftId}`);
  else console.warn(`  Could not delete draft ${draftId}: ${r.status}`);
}

// --- STEP 2: Canvas composite helpers ----------------------------------------
...
try {
  for (const c of CAMPAIGNS) {
    console.log(`\n[${c.key}] Creating draft & generating via test-style-d...`);
    
    // Create temp draft
    const draftId = await createDraft(c);
    createdIds.push(draftId);

    // Fire through test-style-d (exactly as Cover Review page does)
    const coverUrl = await generateCover(draftId);

    // Map URL to local file path
    const localPath = coverUrl.startsWith('/') ? coverUrl.slice(1) : coverUrl;
    console.log(`  Reading file: ${localPath}`);
    if (!fs.existsSync(localPath)) throw new Error(`File not found: ${localPath}`);

    // Save a copy as the background
    const rawOut = `${OUT}/bg-testd-${c.key}.png`;
    fs.copyFileSync(localPath, rawOut);
    console.log(`  ✓ Background saved: ${rawOut}`);

    // Composite all 3 sizes
  ...
  // Cleanup temp drafts
  console.log('\nCleaning up temp drafts...');
  for (const id of createdIds) await deleteDraft(id);
}
